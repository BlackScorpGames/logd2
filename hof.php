...
OutputClass::addnav("Warrior Rankings");
OutputClass::addnav("Dragon Kills", "hof.php?op=kills&subop=$subop&page=1");
OutputClass::addnav("Gold", "hof.php?op=money&subop=$subop&page=1");
OutputClass::addnav("Gems", "hof.php?op=gems&subop=$subop&page=1");
OutputClass::addnav("Charm", "hof.php?op=charm&subop=$subop&page=1");
OutputClass::addnav("Toughness", "hof.php?op=tough&subop=$subop&page=1");
OutputClass::addnav("Resurrections", "hof.php?op=resurrects&subop=$subop&page=1");
OutputClass::addnav("Dragon Kill Speed", "hof.php?op=days&subop=$subop&page=1");
OutputClass::addnav("Sorting");
OutputClass::addnav("Best", "hof.php?op=$op&subop=most&page=$page");
OutputClass::addnav("Worst", "hof.php?op=$op&subop=least&page=$page");
if ($totalplayers > $playersperpage) {
	...
	for($i = 0; $i < $totalplayers; $i+= $playersperpage) {
		...
		if ($page == $pnum) {
			OutputClass::addnav(array("`b`#Page %s`0 (%s-%s)`b", $pnum, $min, $max), "hof.php?op=$op&subop=$subop&page=$pnum");
		} else {
			OutputClass::addnav(array("Page %s (%s-%s)", $pnum, $min, $max), "hof.php?op=$op&subop=$subop&page=$pnum");
		}
	}
}
		...
		$data_header=false, $tag=false, $translate=false)
{
	...
	if ($totalplayers > $playersperpage) {
		OutputClass::output("`c`b`^%s`0`b `7(Page %s: %s-%s of %s)`0`c`n", $title, $page, $from, $to, $totalplayers);
	} else {
...
if (isset($table) && is_array($table)){
	...
	if (isset($me) && $me>"" && $totalplayers){
		...
		if ($pct < 1) $pct = 1;
		OutputClass::output("`c`7You rank within around the top `&%s`7%% in this listing.`0`c",$pct);
	}
}
