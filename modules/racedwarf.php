...
function racedwarf_dohook($hookname,$args){
	...
	case "changesetting":
		...
		break;
	case "chooserace":
		OutputClass::output("<a href='newday.php?setrace=$race$resline'>Deep in the subterranean strongholds of %s</a>, home to the noble and fierce `#Dwarven`0 people whose desire for privacy and treasure bears no resemblance to their tiny stature.`n`n", $city, true);
		OutputClass::addnav("`#Dwarf`0","newday.php?setrace=$race$resline");
		OutputClass::addnav("","newday.php?setrace=$race$resline");
		break;
	case "setrace":
		if ($session['user']['race']==$race){
	...
	case "travel":
		...
		if ($session['user']['location']==$capital){
			OutputClass::addnav("Safer Travel");
			OutputClass::addnav(array("%s?Go to %s", $hotkey, $city),"runmodule.php?module=cities&op=travel&city=$ccity");
		}elseif ($session['user']['location']!=$city){
			OutputClass::addnav("More Dangerous Travel");
			OutputClass::addnav(array("%s?Go to %s", $hotkey, $city),"runmodule.php?module=cities&op=travel&city=$ccity&d=1");
		}
		if ($session['user']['superuser'] & SU_EDIT_USERS){
			OutputClass::addnav("Superuser");
			OutputClass::addnav(array("%s?Go to %s", $hotkey, $city),"runmodule.php?module=cities&op=travel&city=$ccity&su=1");
		}
		Translator::tlschema();
		break;
	case "villagetext":
	...
	case "village":
		if ($session['user']['location'] == $city) {
			...
			OutputClass::addnav($args['tavernnav']);
			Translator::tlschema();
			OutputClass::addnav("K?Great Kegs of Ale","runmodule.php?module=racedwarf&op=ale");
		}
		break;
	case "drinks-text":
...
function racedwarf_run(){
	...
	case "ale":
		...
		PageParts::page_header("Great Kegs of Ale");
