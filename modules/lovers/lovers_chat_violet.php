...
function lovers_chat_violet(){
	...
	if (Http::httpget('act')==""){
		...
		OutputClass::output("You go over to %s`0 and help her with the drinks she is carrying.", getsetting("barmaid", "`%Violet"));
		...
		OutputClass::output("You and %s`0 gossip quietly for a few minutes about not much at all.", getsetting("barmaid", "`%Violet"));
		...
		OutputClass::output("After a few minutes, %s`0 begins to cast burning looks your way, and you decide you had best let %s`0 get back to work.",getsetting('barkeep','`tCedrik'), getsetting("barmaid", "`%Violet"));
	}else if(Http::httpget('act')=="fat"){
		$charm = $session['user']['charm']+e_rand(-1,1);
		OutputClass::output("%s`0 looks you up and down very seriously.", getsetting("barmaid", "`%Violet"));
