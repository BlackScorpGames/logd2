...
class Modules
{
    ...
    public static function get_module_info($shortname){
        ...
        if(Modules::injectmodule($shortname,true)) {
    ...
    public static function modulehook($hookname, $args = false, $allowinactive = false, $only = false)
    {
        ...
        while ($row = db_fetch_assoc($result)) {
            ...
            if (Modules::injectmodule($row['modulename'], $allowinactive)) {
                ...
                if ($cond == "" || module_condition($cond) == true) {
                    ...
                    if (function_exists($row['function'])) {
                        ...
                        trigger_error("Unknown function {$row['function']} for hoookname $hookname in module {$row['module']}.",
                            E_USER_WARNING);
                    }
                    /*******************************************************/
                    ...
                    if (!is_array($res)) {
                        trigger_error("<b>{$row['function']}</b> did not return an array in the module <b>{$row['modulename']}</b> for hook <b>$hookname</b>.",
                            E_USER_WARNING);
                        $res = $args;
                    }
                    if ($testout > "" &&
...
function module_wipehooks() {
	...
	$result = db_query($sql);
	while ($row = db_fetch_assoc($result)){
		DataCache::invalidatedatacache("hook-".$row['location']);
	}
...
function module_addeventhook($type, $chance){
	...
	db_query($sql);
	DataCache::invalidatedatacache("event-".$type);
}
...
function module_do_event($type, $module, $allowinactive=false, $baseLink=false)
{
	...
	$mod = $mostrecentmodule;
	$_POST['i_am_a_hack'] = 'true';
	if(Modules::injectmodule($module, $allowinactive)) {
		$oldnavsection = $navsection;
		Translator::tlschema("module-$module");
