...
function report_companion_move($companion, $activate="fight") {
	...
	if ($activate == "fight" && isset($companion['abilities']['fight']) && $companion['abilities']['fight'] == true && $companion['used'] == false) {
		...
		if ($damage_done==0){
			...
			OutputClass::output("`^%s`4 tries to hit %s but %s `\$RIPOSTES`4 for `^%s`4 points of damage!`n",$companion['name'],$badguy['creaturename'], $badguy['creaturename'], abs($damage_done));
			$companion['hitpoints']+=$damage_done;
		}else{
			OutputClass::output("`^%s`4 hits %s for `^%s`4 points of damage!`n",$companion['name'],$badguy['creaturename'],$damage_done);
			$badguy['creaturehealth']-=$damage_done;
		}

		if ($badguy['creaturehealth'] >= 0) {
			if ($damage_received==0){
				OutputClass::output("`^%s`4 tries to hit `\$%s`4 but `^MISSES!`n",$badguy['creaturename'], $companion['name']);
			}else if ($damage_received<0){
				OutputClass::output("`^%s`4 tries to hit `\$%s`4 but %s `^RIPOSTES`4 for `^%s`4 points of damage!`n",$badguy['creaturename'], $companion['name'], $companion['name'], abs($damage_received));
				$badguy['creaturehealth']+=$damage_received;
			}else{
				OutputClass::output("`^%s`4 hits `\$%s`4 for `\$%s`4 points of damage!`n",$badguy['creaturename'],$companion['name'],$damage_received);
		...
		if ($target === false) {
			...
			if (isset($newenemies[$target])) {
				...
				if ($newenemies[$target]['dead'] == false) {
					...
					OutputClass::output("`^%s`2 heal `^%s`2 for `^%s`2 hitpoints.", $badguy['creaturename'], $newenemies[$target]['creaturename'], $amount);
				}
			}else{
				if ($enemies[$target]['dead'] == false) {
					$enemies[$target]['creaturehealth'] += $amount;
					OutputClass::output("`^%s`2 heal `^%s`2 for `^%s`2 hitpoints.", $badguy['creaturename'], $enemies[$target]['creaturename'], $amount);
				}
			}
		}
	}
