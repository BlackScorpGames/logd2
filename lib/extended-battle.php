...
class ExtendedBattle
{
     ...
     */
    public static function report_companion_move($companion, $activate = "fight")
    {
        ...
        if ($activate == "fight" && isset($companion['abilities']['fight']) && $companion['abilities']['fight'] == true && $companion['used'] == false) {
            ...
            if ($damage_done == 0) {
                OutputClass::output("`^%s`4 tries to hit %s but `\$MISSES!`n", $companion['name'], $badguy['creaturename']);
            } else {
                if ($damage_done < 0) {
                    OutputClass::output("`^%s`4 tries to hit %s but %s `\$RIPOSTES`4 for `^%s`4 points of damage!`n",
                        $companion['name'], $badguy['creaturename'], $badguy['creaturename'], abs($damage_done));
                    $companion['hitpoints'] += $damage_done;
                } else {
                    OutputClass::output("`^%s`4 hits %s for `^%s`4 points of damage!`n", $companion['name'],
                        $badguy['creaturename'], $damage_done);
            ...
            if ($badguy['creaturehealth'] >= 0) {
                if ($damage_received == 0) {
                    OutputClass::output("`^%s`4 tries to hit `\$%s`4 but `^MISSES!`n", $badguy['creaturename'],
                        $companion['name']);
                } else {
                    if ($damage_received < 0) {
                        OutputClass::output("`^%s`4 tries to hit `\$%s`4 but %s `^RIPOSTES`4 for `^%s`4 points of damage!`n",
                            $badguy['creaturename'], $companion['name'], $companion['name'], abs($damage_received));
                        $badguy['creaturehealth'] += $damage_received;
                    } else {
                        OutputClass::output("`^%s`4 hits `\$%s`4 for `\$%s`4 points of damage!`n", $badguy['creaturename'],
                            $companion['name'], $damage_received);
            ...
            if ($activate == "heal" && isset($companion['abilities']['heal']) && $companion['abilities']['heal'] == true && $companion['used'] == false) {
                ...
                if ($session['user']['hitpoints'] < $session['user']['maxhitpoints']) {
                    $hptoheal = min($companion['abilities']['heal'],
                        $session['user']['maxhitpoints'] - $session['user']['hitpoints']);
                    ...
                    $msg = SubstituteClass::substitute_array("`)" . $msg . "`0`n", array("{companion}", "{damage}"),
                        array($companion['name'], $hptoheal));
                    ...
                    foreach ($mynewcompanions as $myname => $mycompanion) {
                        if ($mycompanion['hitpoints'] >= $mycompanion['maxhitpoints'] || $healed || (isset($companion['cannotbehealed']) && $companion['cannotbehealed'] == true)) {
                            ...
                            $hptoheal = min($companion['abilities']['heal'],
                                $mycompanion['maxhitpoints'] - $mycompanion['hitpoints']);
                            ...
                            $msg = SubstituteClass::substitute_array("`)" . $msg . "`0`n",
                                array("{companion}", "{damage}", "{target}"),
                                array($companion['name'], $hptoheal, $mycompanion['name']));
                            Translator::tlschema(isset($companion['schema']) ? $companion['schema'] : "battle");
                            OutputClass::output($msg);
                            Translator::tlschema();
                            $healed = true;
                ...
                if ($badguy['creaturehealth'] >= 0) {
                    if ($damage_received == 0) {
                        ...
                        if ($damage_received < 0) {
                            ...
                            OutputClass::output("`^%s`4 hits `\$%s`4 for `\$%s`4 points of damage!`n",
                                $badguy['creaturename'], $companion['name'], $damage_received);
                            $companion['hitpoints'] -= $damage_received;
                        }
                    }
                }
                ...
                if ($activate == "defend" && isset($companion['abilities']['defend']) && $companion['abilities']['defend'] == true && $defended == false && $companion['used'] == false) {
                    ...
                    $damage_received = $roll['selfdmg'];
                    if ($damage_done == 0) {
                        OutputClass::output("`^%s`4 tries to hit %s but `^MISSES!`n", $companion['name'],
                            $badguy['creaturename']);
                    } else {
                        if ($damage_done < 0) {
                            OutputClass::output("`^%s`4 tries to hit %s but %s `^RIPOSTES`4 for `^%s`4 points of damage!`n",
                    ...
                    if ($activate == "magic" && isset($companion['abilities']['magic']) && $companion['abilities']['magic'] == true && $companion['used'] == false) {
                        ...
                        if ($damage_done == 0) {
                            ...
                            $msg = SubstituteClass::substitute_array("`)" . $msg . "`0`n", array("{companion}"),
                                array($companion['name']));
                            Translator::tlschema(isset($companion['schema']) ? $companion['schema'] : "battle");
                            OutputClass::output($msg);
                            Translator::tlschema();
                        } else {
                            ...
                            $msg = SubstituteClass::substitute_array("`)" . $msg . "`0`n", array("{companion}", "{damage}"),
                                array($companion['name'], $damage_done));
                            Translator::tlschema(isset($companion['schema']) ? $companion['schema'] : "battle");
                            OutputClass::output($msg);
                            Translator::tlschema();
                            $badguy['creaturehealth'] -= $damage_done;
    ...
    public static function show_enemies($enemies)
    {
        ...
        foreach ($enemies as $index => $badguy) {
            ...
            if ($session['user']['alive']) {
                OutputClass::output("%s%s%s%s's Hitpoints%s (Level %s): `6%s`0`n", $ccode,
                    (isset($badguy['istarget']) && $badguy['istarget'] && $enemycounter > 1) ? "*" : "",
                    $badguy['creaturename'], $ccode, $ccode, $badguy['creaturelevel'],
                    $badguy['creaturehealth'] > 0 ? $health : Translator::translate_inline("`7DEAD`0"));
            } else {
                OutputClass::output("`2%s`2's Soulpoints: `6%s`0`n", $badguy['creaturename'],
                    $badguy['creaturehealth'] > 0 ? $health : Translator::translate_inline("`7DEFEATED`0"));
            }
        }
        if ($session['user']['alive']) {
            OutputClass::output("`2YOUR Hitpoints: `6%s`0`n", $session['user']['hitpoints']);
        } else {
            OutputClass::output("`2YOUR Soulpoints: `6%s`0`n", $session['user']['hitpoints']);
        }
    }
    ...
    public static function autosettarget($localenemies)
    {
        ...
        if (is_array($localenemies)) {
            foreach ($localenemies as $index => $badguy) {
                $localenemies[$index] += array(
                    "dead" => false,
                    "istarget" => false
                ); // This line will add these two indices if they haven't been set.
...
function battle_spawn($creature)
{
    ...
    if (is_numeric($creature)) {
        ...
        if ($row = db_fetch_assoc($result)) {
            ...
            OutputClass::output("`^%s`2 summons `^%s`2 for help!`n", $badguy['creaturename'], $row['creaturename']);
        }
    } else {
...
function battle_heal($amount, $target = false)
{
    ...
    if ($amount > 0) {
        if ($target === false) {
            ...
            OutputClass::output("`^%s`2 heals itself for `^%s`2 hitpoints.", $badguy['creaturename'], $amount);
        } else {
            if (isset($newenemies[$target])) {
                ...
                if ($newenemies[$target]['dead'] == false) {
                    ...
                    OutputClass::output("`^%s`2 heal `^%s`2 for `^%s`2 hitpoints.", $badguy['creaturename'],
                        $newenemies[$target]['creaturename'], $amount);
                }
            } else {
                if ($enemies[$target]['dead'] == false) {
                    $enemies[$target]['creaturehealth'] += $amount;
                    OutputClass::output("`^%s`2 heal `^%s`2 for `^%s`2 hitpoints.", $badguy['creaturename'],
                        $enemies[$target]['creaturename'], $amount);
                }
            }
        }
    }
