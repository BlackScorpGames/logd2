        ...
        while (list($buffname, $buff) = each($session['bufflist'])) {
            if (!isset($buff['fields_calculated'])) {
                while (list($property, $value) = each($buff)) {
                    ...
                    $value = preg_replace("/<([A-Za-z0-9]+)\\|([A-Za-z0-9]+)>/", "get_module_pref('\\2','\\1')",
                        $value);
                            ...
                            if (!isset($debuggedbuffs[$buffname][$property])) {
                                if ($errors == "") {
                                    ...
                                    OutputClass::debug("Buffs[$buffname][$property] has an evaluation error<br>"
                                        . htmlentities($origstring, ENT_COMPAT,
                                            Settings::getsetting("charset", "ISO-8859-1")) . " becomes <br>"
                                        . htmlentities($value, ENT_COMPAT,
                                            Settings::getsetting("charset", "ISO-8859-1")) . "<br>"
                                        . $errors);
                                    $val = "";
                                }
