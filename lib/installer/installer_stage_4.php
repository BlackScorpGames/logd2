...
if (!$link){
	...
	OutputClass::output("`^Yahoo, I was able to connect to the database server!");
	OutputClass::output("`2This means that the database server address, database username, and database password you provided were probably accurate, and that your database server is running and accepting connections.`n");
	OutputClass::output("`nI'm now going to attempt to connect to the LoGD database you provided.`n");
	...
	if (!db_select_db($session['dbinfo']['DB_NAME'])){
		OutputClass::output("`\$Rats!  I was not able to connect to the database.");
		$error = db_error();
		if ($error=="Unknown database '{$session['dbinfo']['DB_NAME']}'"){
			OutputClass::output("`2It looks like the database for LoGD hasn't been created yet.");
			OutputClass::output("I can attempt to create it for you if you like, but in order for that to work, the account you provided has to have permissions to create a new database.");
			OutputClass::output("If you're not sure what this means, it's safe to try to create this database, but you should double check that you've typed the name correctly by returning to the previous stage before you try it.`n");
			OutputClass::output("`nTo try to create the database, <a href='installer.php?stage=4&op=trycreate'>click here</a>.`n",true);
		}else{
		...
		OutputClass::output("`n`@Tests`2`n");
		OutputClass::output("I'm now going to run a series of tests to determine what the permissions of this account are.`n");
		$issues = array();
		OutputClass::output("`n`^Test: `#Creating a table`n");
		...
		if ($error=db_error()){
			...
			OutputClass::output("`n`^Test: `#Modifying a table`n");
		...
		if ($error=db_error()){
			...
			OutputClass::output("`n`^Test: `#Creating an index`n");
		...
		if ($error=db_error()){
			...
			OutputClass::output("`n`^Test: `#Inserting a row`n");
		...
		if ($error=db_error()){
			...
			OutputClass::output("`n`^Test: `#Selecting a row`n");
		...
		if ($error=db_error()){
			...
			OutputClass::output("`n`^Test: `#Updating a row`n");
		...
		if ($error=db_error()){
			...
			OutputClass::output("`n`^Test: `#Deleting a row`n");
		...
		if ($error=db_error()){
			...
			OutputClass::output("`n`^Test: `#Locking a table`n");
		...
		if ($error = db_error()) {
			...
			OutputClass::output("`n`^Test: `#Unlocking a table`n");
		$sql = "UNLOCK TABLES";
		db_query($sql);
		if ($error = db_error()) {
			OutputClass::output("`2Result: `\$Fail`n");
			...
			OutputClass::output("`n`^Test: `#Deleting a table`n");
		...
		OutputClass::output("`n`^Test: `#Checking datacache`n");
		if (!$session['dbinfo']['DB_USEDATACACHE']) {
			OutputClass::output("-----skipping, not selected-----`n");
		} else {
		...
		OutputClass::output("`n`^Overall results:`2`n");
		if (count($issues)==0){
			...
			OutputClass::output("I'm sorry, this is not something I can do for you.");
		}
	}
}
?>