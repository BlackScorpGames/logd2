...
// This contains functions to support pvp
    public static function setup_target($name)
    {
        ...
        if (db_num_rows($result) > 0) {
            ...
            if (abs($session['user']['level'] - $row['creaturelevel']) > 2) {
                OutputClass::output("`\$Error:`4 That user is out of your level range!");
                return false;
                ...
                OutputClass::output("`\$Error:`4 That user is not alive.");
                return false;
            ...
            OutputClass::output("`\$Error:`4 That user was not found!  It's likely that their account expired just now.");
            return false;
        }
        return false;
    }
    ...
    public static function pvpvictory($badguy, $killedloc, $options)
    {
        ...
        if ($expbonus > 0) {
            OutputClass::output("`#***Because of the difficult nature of this fight, you are awarded an additional `^%s`# experience!`n",
                $expbonus);
        } else {
            if ($expbonus < 0) {
                OutputClass::output("`#***Because of the simplistic nature of this fight, you are penalized `^%s`# experience!`n",
                    abs($expbonus));
            }
        }
        $wonexp = $exp + $expbonus;
...
//	debuglog("gained $winamount ({$badguy['creaturegold']} base) gold and $wonexp exp (loser lost $lostexp) for killing ", $badguy['acctid']);
        ...
        $mailmessage = array(
            ...
            $session['user']['name'],
            $session['user']['weapon'],
            $badguy['playerstarthp'],
            $session['user']['hitpoints'],
            Settings::getsetting("pvpdeflose", 5),
            $lostexp,
            $badguy['creaturegold'],
            $args['pvpmessageadd'],
            $killedloc,
    ...
    public static function pvpdefeat($badguy, $killedloc, $taunt, $options)
    {
        global $session;
        ...
        if ($row['level'] < $badguy['creaturelevel']) {
            ...
            OutputClass::output("`cThis player has leveled down!!!`c");
