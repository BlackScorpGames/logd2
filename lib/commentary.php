...
function viewcommentary($section,$message="Interject your own commentary?",$limit=10,$talkline="says",$schema=false) {
	...
	if ($doublepost) OutputClass::output("`\$`bDouble post?`b`0`n");
	if ($emptypost) OutputClass::output("`\$`bWell, they say silence is a virtue.`b`0`n");
	...
	for (;$i>=0;$i--){
		...
		if ($moderating){
			if ($session['user']['superuser'] & SU_EDIT_USERS){
				...
				OutputClass::addnav("","user.php?op=setupban&userid=$auth[$i]&reason=".rawurlencode($rawc[$i]));
			}else{
			...
			if ($session['user']['superuser'] & SU_EDIT_COMMENTS) {
				...
				OutputClass::addnav("",$return.$one."removecomment={$commentids[$i]}&section=$section&returnpath=".URLEncode($return)."");
			}
			$out.=$op[$i];
	...
	if ($moderating){
		$scriptname=substr($_SERVER['SCRIPT_NAME'],strrpos($_SERVER['SCRIPT_NAME'],"/")+1);
		OutputClass::addnav("","$scriptname?op=commentdelete&return=".URLEncode($_SERVER['REQUEST_URI']));
	...
	//OutputClass::output the comments
	...
	if ($session['user']['loggedin']) {
		...
		if (array_key_exists("mute",$args) && $args['mute'] &&
			...
			$message="`n`@$message`n";
			OutputClass::output($message);
			OutputClass::output("Sorry, you've exhausted your posts in this section for now.`0`n");
		}
	}
