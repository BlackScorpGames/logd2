            ...
            switch ($info[1]) {
                ...
                case "theme":
                    ...
                    foreach ($skins as $skin) {
                        if ($skin == $row[$key]) {
                            OutputClass::rawoutput("<option value='$skin' selected>" . htmlentities(substr($skin, 0,
                                    strpos($skin, ".htm")), ENT_COMPAT,
                                    Settings::getsetting("charset", "ISO-8859-1")) . "</option>");
                        } else {
                            OutputClass::rawoutput("<option value='$skin'>" . htmlentities(substr($skin, 0,
                                    strpos($skin, ".htm")), ENT_COMPAT,
                                    Settings::getsetting("charset", "ISO-8859-1")) . "</option>");
                        }
                    }
                ...
                case "location":
                    ...
                    foreach ($vloc as $loc => $val) {
                        if ($loc == $row[$key]) {
                            OutputClass::rawoutput("<option value='$loc' selected>" . htmlentities($loc, ENT_COMPAT,
                                    Settings::getsetting("charset", "ISO-8859-1")) . "</option>");
                        } else {
                            OutputClass::rawoutput("<option value='$loc'>" . htmlentities($loc, ENT_COMPAT,
                                    Settings::getsetting("charset", "ISO-8859-1")) . "</option>");
                        }
                ...
                case "dayrange":
                    ...
                    if ($cur && $cur < date("Y-m-d H:i:s", $start)) {
                        OutputClass::rawoutput("<option value='$cur' selected>" . htmlentities($cur, ENT_COMPAT,
                                Settings::getsetting("charset", "ISO-8859-1")) . "</option>");
                    }
                    for ($j = $start; $j < $end; $j = strtotime($step, $j)) {
                        $d = date("Y-m-d H:i:s", $j);
                        OutputClass::rawoutput("<option value='$d'" . ($cur == $d ? " selected" : "") . ">" . HTMLEntities("$d",
                                ENT_COMPAT, Settings::getsetting("charset", "ISO-8859-1")) . "</option>");
                    }
                ...
                case "range":
                    ...
                    for ($j = $min; $j <= $max; $j += $step) {
                        OutputClass::rawoutput("<option value='$j'" . ($row[$key] == $j ? " selected" : "") . ">" . HTMLEntities("$j",
                                ENT_COMPAT, Settings::getsetting("charset", "ISO-8859-1")) . "</option>");
                    }
                    OutputClass::rawoutput("</select>");
                    break;
                case "floatrange":
                    ...
                    for ($j = $min; $j <= $max; $j = round($j + $step, 2)) {
                        OutputClass::rawoutput("<option value='$j'" . ($val == $j ? " selected" : "") . ">" . HTMLEntities("$j",
                                ENT_COMPAT, Settings::getsetting("charset", "ISO-8859-1")) . "</option>", true);
                    }
                ...
                case "bitfield":
                    ...
                    while (list($k, $v) = each($info)) {
                        ...
                        list($k, $v) = each($info);
                        if (!$pretrans) {
                            $v = Translator::translate_inline($v);
                        }
                ...
                case "datelength":
                    ...
                    Translator::tlschema("ShowFormClass::showform");
                    ...
                    OutputClass::rawoutput("<select name='$keyout'>");
                    while (list($k, $v) = each($vals)) {
                        OutputClass::rawoutput("<option value=\"" . htmlentities($v, ENT_COMPAT,
                                Settings::getsetting("charset",
                                    "ISO-8859-1")) . "\"" . ($row[$key] == $v ? " selected" : "") . ">" . htmlentities($v,
                                ENT_COMPAT, Settings::getsetting("charset", "ISO-8859-1")) . "</option>");
                    }
                    OutputClass::rawoutput("</select>");
                    break;
                ...
                case "enum":
                    ...
                    while (list($k, $v) = each($info)) {
                        ...
                        $select .= ("<option value='$optval'" . ($selected ? " selected" : "") . ">" . HTMLEntities("$optdis",
                                ENT_COMPAT, Settings::getsetting("charset", "ISO-8859-1")) . "</option>");
                    }
                    $select .= "</select>";
                    OutputClass::rawoutput($select);
                ...
                case "password":
                    ...
                    OutputClass::rawoutput("<input type='password' name='$keyout' value='" . HTMLEntities($out,
                ...
                case "hidden":
                    if (isset($row[$key])) {
                        OutputClass::rawoutput("<input type='hidden' name='$keyout' value=\"" . HTMLEntities($row[$key],
                                ENT_COMPAT,
                                Settings::getsetting("charset", "ISO-8859-1")) . "\">" . HTMLEntities($row[$key],
                                ENT_COMPAT, Settings::getsetting("charset", "ISO-8859-1")));
                    }
                    break;
                case "viewonly":
                ...
                case "textarea":
                    ...
                    if (isset($resize) && $resize) {
                        ...
                        OutputClass::rawoutput("<textarea id='textarea$key' class='input' name='$keyout' cols='$cols' rows='5'>" . htmlentities(str_replace("`n",
                                "\n", $text), ENT_COMPAT,
                                Settings::getsetting("charset", "ISO-8859-1")) . "</textarea>");
                ...
                case "int":
                    ...
                    OutputClass::rawoutput("<input name='$keyout' value=\"" . HTMLEntities($out, ENT_COMPAT,
                            Settings::getsetting("charset", "ISO-8859-1")) . "\" size='5'>");
                    break;
                case "float":
                    OutputClass::rawoutput("<input name='$keyout' value=\"" . htmlentities($row[$key], ENT_COMPAT,
                            Settings::getsetting("charset", "ISO-8859-1")) . "\" size='8'>");
                    break;
                case "string":
                    $len = 50;
                    $minlen = 50;
                    ...
                    OutputClass::rawoutput("<input size='$minlen' maxlength='$len' name='$keyout' value=\"" . HTMLEntities($val,
                            ENT_COMPAT, Settings::getsetting("charset", "ISO-8859-1")) . "\">");
                    break;
                default:
                    if (array_key_exists($info[1], $extensions)) {
                        ...
                        OutputClass::rawoutput("<input size='50' name='$keyout' value=\"" . HTMLEntities($val,
                                ENT_COMPAT, Settings::getsetting("charset", "ISO-8859-1")) . "\">");
                    }
            }
            OutputClass::rawoutput("</td></tr>", true);
