...
function pvplist($location=false,$link=false,$extra=false,$sql=false){
	...
	if ($num != 0 && (!isset($loc_counts[$location]) ||
		...
		OutputClass::output("`n`n`&As you listen to different people around you talking, you glean the following additional information:`n");
		foreach ($loc_counts as $loc=>$count) {
			...
			if ($count == 1) {
			OutputClass::output("`&There is `^%s`& person sleeping in %s whom you might find interesting.`0`n", $count, $loc);
			} else {
			OutputClass::output("`&There are `^%s`& people sleeping in %s whom you might find interesting.`0`n", $count, $loc);
			}
		}
	}
	Translator::tlschema();
