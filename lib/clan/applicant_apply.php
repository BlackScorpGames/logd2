<?php
		...
		if ($to>0){
			OutputClass::output("`%%s`7 accepts your application, files it in her out box, and folds her hands on the desk, staring at you.",$registrar);
			OutputClass::output("You stand there staring blankly back at her for a few minutes before she suggests that perhaps you'd like to take a seat in the waiting area.");
			...
			while ($row = db_fetch_assoc($result)){
				...
				SystemMailClass::systemmail($row['acctid'],$apply_subj,$msg);
			}
			...
			if ( nltoappon($row['clandesc']) != "" ) {
				...
				SystemMailClass::systemmail($session['user']['acctid'],array($subject),array($mail, $row['clanname'], $row['clanshort'], nltoappon($row['clandesc'])));
			}
		}else{
			...
			if (db_num_rows($result)>0){
				OutputClass::output("`7You ask %s`7 for a clan membership application form.",$registrar);
				OutputClass::output("She opens a drawer in her desk and pulls out a form.  It contains only two lines: Name and Clan Name.");
				OutputClass::output("You furrow your brow, not sure if you really like having to deal with all this red tape, and get set to concentrate really hard in order to complete the form.");
				OutputClass::output("Noticing your attempt to write on the form with your %s, %s`7 claims the form back from you, writes %s`7 on the first line, and asks you the name of the clan that you'd like to join:`n`n",$session['user']['weapon'],$registrar,$session['user']['name']);
...
/*//*/					if (isset($row['handled']) && $row['handled']) continue;
				...
				OutputClass::output("`7You ask %s`7 for a clan membership application form.",$registrar);
				OutputClass::output("She stares at you blankly for a few moments, then says, \"`5Sorry pal, no one has had enough gumption to start up a clan yet.  Maybe that should be you, eh?`7\"");
