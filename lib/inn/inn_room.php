...
if ($pay){
	...
	if ($session['user']['boughtroomtoday']){
		...
		if ($session['user']['goldinbank'] >= $bankexpense && $bankexpense != $expense) {
			OutputClass::output("And since you are such a fine person, I'll even offer you a rate of `\$%s`0 gold if you pay direct from the bank.", $bankexpense);
			if (strpos($fee, "%")) {
				OutputClass::output("That includes a %s transaction fee.", $fee);
			} else {
				OutputClass::output("That includes a transaction fee of %s gold.",
						$fee);
			}
		}
		...
		OutputClass::output("`n`n\"Also, let me tell you about our new 'Bodyguard Assistance Program' &#151; BAP.  You see, you hire one of my guards here, and they'll protect you should anyone happen to, er, pick the locks of your room,\" he says as he gestures to a series of men sitting at one of the inn's tables drinking ale.", true);
		OutputClass::output("They range in size from a skinny shifty-eyed fellow who appears barely able to lift his stein to a great bear of a fellow.");
		OutputClass::output("This bruiser has a tattoo of a heart with \"Mom\" written across it on his huge bicep, and goes to take a sip from his ale, but instead crushes his stein, squirting it all over the skinny fellow who doesn't voice any objection for obvious reasons.");
		OutputClass::output("\"We call it the BAP program because when someone tries to sneak into your room, BAP BAP BAP, our guys go to work.");
		OutputClass::output("There's only two conditions: you pay your fee up front, and the guard you choose gets to keep a portion of the rewards from any fights.\"");
		OutputClass::output("`n`nNot wanting to part with your money when the fields offer a place to sleep, you debate the issue.");
		OutputClass::output("You realize, however, that the inn is a considerably safer place to sleep.");
