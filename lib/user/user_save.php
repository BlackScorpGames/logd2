	...
	if (isset($userinfo[$key])){
		if ($key=="newpassword" ){
			...
			if ((int)$value != (int)$oldvalues['superuser']){
				$sql.="$key = \"$value\",";
				$updates++;
				OutputClass::output("Superuser values have changed.`n");
				if ($session['user']['acctid']==$userid) {
			...
			$sql.="$key = \"$val\",";
			OutputClass::output("Changed player ctitle from %s`0 to %s`0`n", $oldvalues['ctitle'], $tmp);
			$oldvalues[$key]=$tmp;
			if ($newname != $oldvalues['name']) {
				$sql.="name = \"".addslashes($newname)."\",";
...
if ($petition!="")
	OutputClass::addnav("","viewpetition.php?op=view&id=$petition");
