...
class BattleBuffs
{
    public static function process_dmgshield($dshield, $damage)
    {
        ...
        foreach ($dshield as $buff) {
            ...
            if (is_array($msg)) {
                ...
                if ($msg > "") {
                    $msg = SubstituteClass::substitute_array("`)" . $msg . "`0`n", array("{damage}"),
                        array($realdamage));
        ...
        foreach ($session['bufflist'] as $key => $buff) {
                    ...
                    if (is_array($companions) && count($companions) > 0 && $auraeffect != 0) {
                        foreach ($companions as $name => $companion) {
                            ...
                            if ($companion['hitpoints'] < $companion['maxhitpoints'] && ($companion['hitpoints'] > 0 || ($companion['cannotdie'] == true && $auraeffect > 0))) {
                                ...
                                $msg = SubstituteClass::substitute_array("`)" . $buff['auramsg'] . "`0`n",
                                    array("{damage}", "{companion}"), array($hptoregen, $companion['name']));
                                OutputClass::output($msg);
            ...
            if (isset($buff['minioncount']) && $tag == "roundstart" && ((isset($buff['areadamage']) && $buff['areadamage'] == true) || $badguy['istarget'] == true) && $badguy['dead'] == false) {
                ...
                while ($minioncounter <= $buff['minioncount'] && $who >= 0) {
                    ...
                    if (is_array($msg)) {
                        $msg = Translator::sprintf_translate($msg);
                        $msg = SubstituteClass::substitute("`)" . $msg . "`0`n", array("{damage}"),
                            array(abs($damage)));
                        OutputClass::output_notl($msg); //Here it's already translated
                    } else {
                        if ($msg > "") {
    ...
    public static function process_lifetaps($ltaps, $damage)
    {
        global $session, $badguy;
        Translator::tlschema("buffs");
